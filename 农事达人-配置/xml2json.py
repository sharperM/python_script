...
def convert_xls_to_json(input_file, output_file):
    ...
    jsonname2 = ['tab1_list','tab1_banner',"tab2_list",'tab2_list','tab2_list','tab3_list','tab3_list']
    for i in range(len(jsonname)):
        ...
        df['imageUrl'] = r"https://youmieyou.oss-cn-shenzhen.aliyuncs.com/nongshidaren/" + jsonname[i] + "/"+ jsonname2[i] + "_id_" + df['id'].astype(str) + ".webp"
        # df['imageUrl'] = r"file:///android_asset/" + jsonname[i] + "/"+ jsonname[i] + "_id_" + df['id'].astype(str) + ".webp"
        # Shuffle the rows
        shuffled_df = df.sample(frac=1, random_state=42)
        print(shuffled_df)
        json_data = shuffled_df.to_json(orient='records', force_ascii=False)
        # 替换 '\/' 为 '/'
